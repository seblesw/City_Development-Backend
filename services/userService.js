...
const registerUserService = async (
  ...
  if (
    ...
    throw new Error(
      "የተጠቃሚ መረጃዎች (ስም, የአባት ስም, ብሔራዊ መታወቂያ, ጾታ, የጋብቻ ሁኔታ, አስተዳደራዊ ክፍል) መግለጽ አለባቸው።"
    );
  }

  if (isCoOwner && !relationship_type) {
    throw new Error(
      "የጋራ ባለቤት መረጃዎች (ስም, የአባት ስም, ብሔራዊ መታወቂያ, ጾታ, የጋብቻ ሁኔታ, የግንኙነት አይነት) መግለጽ አለባቸው።"
    );
  }
  ...
  const user = await User.create(
    {
      ...
      marital_status,
      administrative_unit_id,
      primary_owner_id: isCoOwner ? userData.primary_owner_id : null,
      relationship_type: isCoOwner ? relationship_type : null,
    },
    { transaction }
  );
...
const updateUserService = async (id, userData) => {
  ...
  try {
    ...
    if (primary_owner_id) {
      const primaryOwner = await User.findByPk(primary_owner_id, {
        transaction,
      });
    ...
    if (marital_status && marital_status !== "ነጠላ") {
      const coOwners = await User.findAll({
    ...
    const updateData = {
      ...
      oversight_office_id:
        oversight_office_id !== undefined
          ? oversight_office_id
          : user.oversight_office_id,
