          ...
          validate: {
            isValidHistory(value) {
              ...
              for (const entry of value) {
                if (!entry.status || !Object.values(APPLICATION_STATUSES).includes(entry.status)) {
        ...
        hooks: {
          beforeCreate: async (application, options) => {
            ...
            application.action_log = [
              { action: "CREATED", changed_by: application.created_by, changed_at: new Date() },
            ];
          },
          beforeUpdate: async (application, options) => {
            ...
            if (application.changed("land_record_id") && application.land_record_id) {
              const landRecord = await db.models.LandRecord.findByPk(application.land_record_id, {
                transaction: options.transaction,
              });
              if (!landRecord) throw new Error("የመሬት መዝገብ አልተገኘም።");
            ...
            if (
              ...
              const documents = await db.models.Document.findOne({
                where: { application_id: application.id, deleted_at: { [Op.eq]: null } },
                transaction: options.transaction,
              });
              if (!documents) {
